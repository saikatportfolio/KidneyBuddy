...
class FeedbackPageState extends State<FeedbackPage> {
  ...
  String _patientPhoneNumber =
      ''; // To store patient's phone number from provider or input
  bool _patientDetailsLoaded =
      false; // New state variable to track if patient details are loaded
      ...
      if (details != null) {
        setState(() {
          ...
          _nameController.text =
              details.name; // Initialize controller even if not shown
          _phoneNumberController.text =
              details.phoneNumber; // Initialize controller even if not shown
        });
        logger.i(
          'Patient details loaded for feedback submission: ${details.name}',
        );
      } else {
        ...
        setState(() {
          ...
          _nameController.text =
              googleUserName ?? ''; // Pre-fill with Google name if available
          _phoneNumberController.text =
              ''; // Leave phone number empty for user input
        });
        logger.w(
          'Patient details not available. Google name pre-filled: $googleUserName',
        );
      }
    });
  }
  ...
  void _submitFeedback() async {
    if (_formKey.currentState!.validate()) {
      ...
      final feedback = FeedbackModel(
        ...
        phoneNumber:
            phoneNumberToSubmit, // Use phone number from controller or fetched details
